...
d3DemoApp.controller('AppCtrl', function AppCtrl ($scope, $http) {
  ...
  $scope.getCommitData = function () {
    $http({
      ...
      //url: './data/a.json'
      url: 'http://django.dsini20.schedar.uberspace.de/api/search/project?format=json&title*istartswith=' + $scope.firstChar
...
let nodes = pack(root).leaves().map(node => {
  ...
  console.log(Math.floor((Math.random() * 10) + 1))
	return {
		...
		cat:cat,
		name: data.title,
		value: sizes[Math.floor((Math.random() * 10) + 1)], //data.value,
		icon: data.logo,
		desc: data.description_de,
	};
});
function ticked() {
	node
...
node.filter(d => {!String(d.icon); console.log(d.icon); }) //.includes('img/') )
	.append('text')
